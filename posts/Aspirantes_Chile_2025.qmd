---
title: "Capital burocrático de los aspirantes a la presidencia de Chile 2025"
author: "Dr. Norberto Bustos"
date: "2025-08-22"
description: "Análisis del capital burocrático de los candidatos presidenciales de Chile en 2025."
image: "/images/Chile_2025.png"
categories: [analisis, politica,capital burocrático, Chile]
format: 
  html: 
    toc: true
    toc-title: "Contenido"
...
editor: visual
params:
  datafile1: "~/Documentos/web_n0dn/n0dn/dataframes/Aspirantes_Chile_2025.csv"
lang: es
fontsize: 12pt
...
## ¿Quiénes son l\@s aspirantes a la presidencia de Chile en 2025?
...
kb_chile <- read.csv(params$datafile1) # Para codificar en windows
tb_aspirantes_kb <- kb_chile %>% 
  dplyr::select(Nombre, Edad_contienda,Genero,Comuna,Coalición,Partido, Activo, kb) %>% 
  rename(Aspirante = Nombre, 
         Edad = Edad_contienda,
         Género = Genero,
         "Comuna de nacimiento" = Comuna,
         "Candidatura activa" = Activo,
         "Capital burocrático\n(kb)" = kb
         ) %>% 
  mutate(Género = case_when(Género == "M"~"Hombre",
                            Género == "F"~"Mujer")) %>% 
  arrange(desc(`Capital burocrático\n(kb)`)) %>% 
   kable(caption = "Elaborado por: n0dn | The linked data") %>%
...
```{r}
#| label: tb2_aspirantes_kb_gt
#| echo: false
#| warning: false
#| message: false
#| fig-cap: Tabla 2. Aspirantes según su capital burocrático
...
# Calcular rango antes
rango_prom_kb <- kb_chile %>%
  dplyr::select(Nombre, Coalición, Activo, kb) %>%
  rename(Aspirante = Nombre, "Candidatura activa" = Activo) %>%
  group_by(Coalición, Aspirante) %>%
  summarise(prom_kb = mean(kb, na.rm = TRUE), .groups = "drop") %>%
  summarise(
    min_val = min(prom_kb, na.rm = TRUE),
    max_val = max(prom_kb, na.rm = TRUE)
  )

# Extraer valores para usarlos después
min_val <- rango_prom_kb$min_val
max_val <- rango_prom_kb$max_val

# Ahora generar la tabla
kb_chile %>%
  dplyr::select(Nombre, Edad_contienda, Genero, Coalición, Activo, kb) %>%
  rename(
    Aspirante = Nombre,
    Edad = Edad_contienda,
    Género = Genero,
  ...
  group_by(Coalición, Aspirante) %>%
  summarise(
    prom_kb = mean(kb, na.rm = TRUE),
    ranking_kb = case_when(
      prom_kb >= 455 ~ "Alto",
      prom_kb >= 440 & prom_kb < 455   ~ "Medio",
      TRUE ~ "Bajo"
    ),
    .groups = "drop"
  ) %>%
  arrange(desc(prom_kb)) %>%
  mutate(
    ranking_icon = case_when(
      ranking_kb == "Alto" ~ "▲",
      ranking_kb == "Medio" ~ "■",
      ranking_kb == "Bajo" ~ "▼"
    )
  ...
  gt() %>%
  tab_header(
    title = "Tabla 2. Aspirantes según su capital burocrático",
    subtitle = "Presidencia de Chile 2025"
  ) %>%
  fmt_number(columns = prom_kb, decimals = 1) %>%
  cols_label(
    prom_kb = "Prom. Kb",
    ranking_kb = "Ranking Kb",
    ranking_icon = "Capital burocrático"
  ) %>%
  cols_align(
    align = "center",
    columns = ranking_icon
  ) %>%
  tab_footnote(
    footnote = "La intensidad del color representa la experiencia en cargos directivos del aspirante con respecto a la media histórica (1818-2022)",
    locations = cells_column_labels(columns = ranking_kb)
  ) %>%
  tab_footnote(
    footnote = "▲ Alto (≥455 kb) | ■ Medio (440-454.99 kb) | ▼ Bajo (≤ 439.99 kb)",
    locations = cells_column_labels(columns = ranking_icon)
  ) %>%
  gt_theme_pff() %>%
  gt_color_box(
    columns = prom_kb,
  ...
  tab_style(
    style = cell_text(color = "#28a745", size = px(20), weight = "bold"),
    locations = cells_body(columns = ranking_icon, rows = ranking_kb == "Alto")
  ) %>%
  tab_style(
    style = cell_text(color = "#ff9800", size = px(20), weight = "bold"),
    locations = cells_body(columns = ranking_icon, rows = ranking_kb == "Medio")
  ) %>%
  tab_style(
    style = cell_text(color = "#dc3545", size = px(20), weight = "bold"),
    locations = cells_body(columns = ranking_icon, rows = ranking_kb == "Bajo")
  )


```

Como se aprecia en @grap_coaliciones_kb los aspirantes de Coalición Unidad por Chile que representa al oficialismo y a los partidos de izquierda y centro izquierda (tanto de la nueva hisquierda encabezada por el presidente Boric como de la extinta Concertación que acumula a los partidos de centro izquierda) acumula principalemente Intra-burocrático con 1,044 kb lo que indica que predomina la experiencia en Ministerios de Estado, seguida de ella se encuentra la sub-burocrática que refiere la experiencia en Partidos Políticos y Alcaldias o Comunas con 589 kb, y en tercer lugar el capital para-burocrático con 567 kb los cuales abarcan principalmente el trabajo legislativo. Finalmente el capital exa-burocrático que incluye actividades empresariales y en sociedad civil con 258 kb. Lo anterior, muestra que los candidatos de la Coalición Unidad por Chile cuentan con amplia experiencia en labores ejecutivas del Estado, cuentan con una base partidista sólida y experiencia en el campo legislativo, todas ellas por encima de las labores empresariales o de sociedad civil.
...
long_kbtyp_suma <- long_kbtyp %>%
  group_by(Coalición, Tipo_de_capital) %>%
...
suma_total_coalicion <- long_kbtyp_suma %>%
  group_by(Coalición) %>%
  summarise(total_kb = sum(cantidad, na.rm = TRUE), .groups = "drop")

## Ordenar levels de Coalición de mayor a menor por total_kb
orden_coalicion <- suma_total_coalicion %>%
  arrange(desc(total_kb)) %>%
  pull(Coalición)

# Aplicar orden al factor en el dataset long_kbtyp_suma
long_kbtyp_suma$Coalición <- factor(long_kbtyp_suma$Coalición, levels = orden_coalicion)

# También en la tabla de sumas para etiquetas
suma_total_coalicion$Coalición <- factor(suma_total_coalicion$Coalición, levels = orden_coalicion)
...
kb_chile$Coalición <- factor(kb_chile$Coalición, levels = unique(kb_chile$Coalición[order(kb_chile$kb)]))
...
ggplot(data = long_kbtyp_suma, aes(x = Coalición, y = cantidad, fill = Tipo_de_capital)) +
  geom_col() +
  ...
  coord_flip() +
  geom_text(data = suma_total_coalicion, 
            aes(x=Coalición, y= total_kb +50,
...
Los vínculos de Mulet se encuentran dentro de una subred de la DC, liderada por Adolfo Zaldivar quien tuvo un papel relevante junto a sus hermanos salidvar para la construcción de ese partido posterior al regreso a la democracia en 1989, pero que por motivos personales decidió abandonar ese partido, fundar el Partido Regionalista Independiente (PRI) y con ello jalar a su subred fuera de la DC, como es el caso de Mulet. De allí que su acción como actor política se funde en la negociación sin cuartel para mantenerse dentro del espectro político. Por su parte, Jeanet Jara quien actualmente es la candidata de la coalición oficialista Unidad por Chile, representa a los primeros cuadros surgidos de la unión de movimientos insurgentes de izquierda de la primera década del siglo XXI y que apoyaron la candidatura de Gabriel Boric. Jara, la segunda candidata a la presidencia del país por el Partido Comunista, despues de Gladys Marín, es una política con la mayor experiencia en cargos dentro del Ministerio del trabajo durante las administraciones de Michelle Bachelet y Gabriel Boric, lo que le permite contar con el respaldo de los partidos de izquierda de la coalición formada por la extinta Concertación y los nuevos partidos que apoyan al presidente Boric.

```{r}
#| label: graph_Dist_cargos
#| echo: false
#| warning: false
#| message: false
#| fig-cap: Gráfico 2. Distribución de kb por aspirante 
...
kb_chile$Nombre <- factor(kb_chile$Nombre, levels = kb_chile$Nombre[order(kb_chile$kb)])
# Apilar datos
datos_long <- kb_chile %>%
  pivot_longer(
...
lineas_kb <- tibble(
  yintercept = c(418.88, 455.17),
  label_text = c("kb 2025 = 418.88", "kb histórico = 455.17"),
...
ggplot(datos_long, aes(x = Nombre, y = cantidad, fill = institucion)) +
  ...
  geom_label(data = lineas_kb,
             aes(x = length(unique(kb_chile$Nombre)), 
                 y = pos_y_superior, 
                 label = label_text), 
  ...
  labs(title = "Aspirantes para la presidencia de Chile, 2025",
...
La diferencia más importante de un político además de su capital burocrático es su centralidad dentro de la red política histórica. La política es el uso instrumental de los vínculos de personajes con capitales social, económico y burocrático lo suficientemente estratéfico para movilizar el capital social de grandes conglomerados. Para mantener la cumulación de dicho capital político los personajes se organizan en torno a diferentes estructuras que dependen de las fuentes de poder que las vinculan. En el caso de los candidatos a la presidencia de Chile para 2025 se encuentran dos tipos de estructuras que compiten entre sí como clases dirigentes:

-   las castas heredada por vínculos familiares, políticos, empresariales que conformaron una clase dirigente durente la dictadura de Pinochet, la Concertación y el gobierno de Sebstián Piñera
